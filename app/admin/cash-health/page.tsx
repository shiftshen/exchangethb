...
function buildHref(status: string, criticalOnly: boolean, page: number) {
...
export default async function AdminCashHealthPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>>; }) {
  ...
  const exportHref = `/api/admin/cash-health/export${buildHref(statusFilter, criticalOnly, 1).replace('/admin/cash-health', '')}`;
    ...
    <main className="container-shell space-y-8 py-10">
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {['all', 'healthy', 'degraded', 'down'].map((status) => (
            <Link key={status} href={buildHref(status, criticalOnly, 1)} className={`rounded-full px-3 py-1 text-xs font-medium ${statusFilter === status ? 'bg-brand-700 text-white' : 'bg-stone-100 text-stone-700'}`}>
              {status}
            </Link>
          ))}
        </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link href={buildHref(statusFilter, false, 1)} className={`rounded-full px-3 py-1 text-xs font-medium ${!criticalOnly ? 'bg-brand-700 text-white' : 'bg-stone-100 text-stone-700'}`}>all alerts</Link>
            <Link href={buildHref(statusFilter, true, 1)} className={`rounded-full px-3 py-1 text-xs font-medium ${criticalOnly ? 'bg-brand-700 text-white' : 'bg-stone-100 text-stone-700'}`}>critical only</Link>
          </div>
        </div>
        <div className="mt-4 space-y-2">
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-stone-600">
          ...
          <div className="flex gap-2">
            <Link href={buildHref(statusFilter, criticalOnly, Math.max(1, safePage - 1))} className="rounded-full border border-stone-300 px-3 py-1">Prev</Link>
            <Link href={buildHref(statusFilter, criticalOnly, Math.min(pageCount, safePage + 1))} className="rounded-full border border-stone-300 px-3 py-1">Next</Link>
          </div>
        </div>
      </div>
    </main>
