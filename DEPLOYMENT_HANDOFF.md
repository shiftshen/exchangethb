...
- Docker, Prisma-backed persistence path with file fallback, long-running worker service, env template, and VPS-oriented deployment files are included.
...
11. Verify `/api/health`, `/th`, `/en`, `/zh`, `/admin/login`, `/sitemap.xml`, `/robots.txt`
...
## Backup and recovery
- Back up `content/admin-config.json`, `content/cash-scrape-cache.json`, and `content/admin-audit-log.json` if you are operating in file fallback mode.
- Back up PostgreSQL and Redis volumes if database-backed persistence is enabled in production.
...
- Confirm `storage.mode` is `database` in health responses if production is expected to use PostgreSQL persistence.
- Confirm `configWarnings` is empty in health responses before go-live.
...
- The compose stack now keeps a long-running worker active for cache refresh; tune `CASH_SCRAPE_INTERVAL_MINUTES` if needed.
- If you need a manual refresh outside the daemon, run `npm run worker`.
